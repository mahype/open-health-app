...
// MARK: - Sensor Detail Content
// Inner view with a date-range-filtered @Query — only the rows for the current
// time window are loaded from SwiftData. The outer SensorDetailView re-creates
// this view (via .id(rangeKey)) whenever the selected range changes, which forces
// @Query to rebuild its predicate against the new window.

struct SensorDetailContent: View {
    let dataType: HealthDataType
    let isBloodPressure: Bool
    let selectedRange: ChartTimeRange

    // Filtered to the selected date window — NOT all rows in the store.
    @Query private var rangeItems: [HealthDataItem]

    // Pre-computed aggregates for this type + window (may be empty on first load).
    @Query private var aggregates: [AggregatedDataPoint]
    ...
    init(
        ...
        self.isBloodPressure = isBloodPressure
        ...
        _rangeItems = Query(
            filter: #Predicate<HealthDataItem> { item in
                item.startDate >= start && item.startDate < end
            },
            sort: \HealthDataItem.startDate
        )

        let typeRaw = isBloodPressure
            ? HealthDataType.bloodPressureSystolic.rawValue
            : dataType.rawValue

        _aggregates = Query(
            filter: #Predicate<AggregatedDataPoint> { agg in
                agg.dataTypeRaw == typeRaw
                    && agg.periodStart >= start
                    && agg.periodStart < end
            },
            sort: \AggregatedDataPoint.periodStart
        )
    }

    // MARK: - Filtered Items

    var filteredItems: [HealthDataItem] {
        if isBloodPressure {
            return rangeItems.filter {
                $0.type == .bloodPressureSystolic || $0.type == .bloodPressureDiastolic
            }
        }
        return rangeItems.filter { $0.type == dataType }
    }

    var hasData: Bool { !filteredItems.isEmpty }
    ...
    private var latestBPText: String {
        let sys = filteredItems.last(where: { $0.type == .bloodPressureSystolic })
        let dia = filteredItems.last(where: { $0.type == .bloodPressureDiastolic })
    ...
    private var lineChart: some View {
        let useLongRange = selectedRange == .sixMonths || selectedRange == .oneYear
        let chartData = SensorDetailViewModel.chartPoints(
            ...
            if useLongRange {
                ForEach(chartData, id: \.date) { point in
                    LineMark(
                        ...
                        y: .value(dataType.displayName, point.value)
                    )
    ...
    private var barChart: some View {
        ...
        let unit: Calendar.Component = (selectedRange == .sixMonths || selectedRange == .oneYear)
            ? .weekOfYear : .day

        return Chart(chartData, id: \.date) { point in
            BarMark(
                x: .value("Datum", point.date, unit: unit),
                y: .value(dataType.displayName, point.value)
            )
            .foregroundStyle(
                LinearGradient(
    ...
    private var bpClassificationCard: some View {
        let latestSys = filteredItems.last(where: { $0.type == .bloodPressureSystolic })?.value ?? 0
        let latestDia = filteredItems.last(where: { $0.type == .bloodPressureDiastolic })?.value ?? 0
    ...
    private var sleepNightCount: Int { sleepNights.count }
    ...
    private var sleepStageAggregates: [(id: String, date: Date, stage: SleepStage, value: Double)] {
        ...
        for (date, dayItems) in grouped {
            ...
            for (stage, stageItems) in byStage {
                ...
                result.append((
                    id: "\(date.timeIntervalSince1970)-\(stage.rawValue)",
                    date: date,
                    stage: stage,
                    value: total
                ))
    ...
    private var sleepStackedChart: some View {
        let aggs = sleepStageAggregates
        return Chart(aggs, id: \.id) { agg in
            BarMark(
    ...
    private var sleepStageBreakdownCard: some View {
        ...
        let stageAverages: [(stage: SleepStage, avg: Double)] = SleepStage.allCases.map { stage in
            let total = byStage[stage]?.map(\.value).reduce(0, +) ?? 0
            return (stage: stage, avg: total / Double(nightCount))
        }
